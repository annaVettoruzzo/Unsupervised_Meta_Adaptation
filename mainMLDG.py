...
from trainingMLDG import MLDG, evaluate_mldg
...
for it in range(3):
    ...
    ############### MODEL ########################
    mldg_model = models.MLDGModel().to(DEVICE)
    mldg_mldg = MLDG(mldg_model, params.loss, lr_inner=0.05).fit(domains_train, steps=params.training_steps)
    torch.save(mldg_mldg, PATH+"model_mldg.pkl")

    ############### EVALUATE ########################
    test_accuracies = evaluate_mldg(mldg_model, domains_test)
    print(f"Mean accuracy: {np.mean(test_accuracies)}")
    print(f"Std accuracy: {np.std(test_accuracies)}")
    write_in_file(test_accuracies, PATH+"perf_MLDG")
